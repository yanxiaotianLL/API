...
namespace myselfFast.Fund.Core
{
    ...
    public class PagedList<T> : List<T>, IPagedList<T> 
    {
        ...
        public PagedList(IEnumerable<T> source, int pageIndex, int pageSize,int total,bool isCal)
        {          
        ...
        private void Init(IEnumerable<T> source, int pageIndex, int pageSize, int? totalCount = null)
        {
            ...
            PageSize = pageSize;
            PageIndex = pageIndex;
            source = totalCount == null ? source.Skip((pageIndex-1) * pageSize).Take(pageSize) : source;
            AddRange(source);
        }
