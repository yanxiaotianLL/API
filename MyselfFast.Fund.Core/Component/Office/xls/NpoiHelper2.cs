...
{
    /**********************************************************************************
     *项目名称	：myselfFast.ZFBZ.Core.Component.Office
     *项目描述  ：
     *类名称    ：NpoiHelper
     *版本号    ：v1.0.0
     *机器名称  ：LIUYONG-PC
    ...
    ***********************************************************************************/
    ...
    /// NPOI辅助工具类
    /// </summary>
    public static class NpoiHelper2
    {
        /// <summary>
        /// 将excle输出到文件
        /// </summary>
            ...
            where T : class
        {
            ...
            //设置单元格样式
            cell.CellStyle = NpoiHelper2.CreateTileCellStyle(workbook, sheet,15);
            //合并单元格
            CellRangeAddress cras = new CellRangeAddress(0, 0, 0, 2);
            sheet.AddMergedRegion(cras);
        }
        public static void FileDataTimeToSheet<T>(XSSFWorkbook workbook, ExportModle<T> dataList, ISheet sheet)
      where T : class
        {
            //填充表头部分
            int count = dataList.Propertys.Count;
            ...
            var cell = row.CreateCell(0);
            //设置单元格值
            cell.SetCellValue("导出时间："+DateTime.Now.ToString());
            //设置单元格样式
            ...
            //设置边框
            NpoiHelper2.SetBorderForAll(style);
            cell.CellStyle = style;
            //合并单元格
            CellRangeAddress cras = new CellRangeAddress(1, 1, 0, 2);
            sheet.AddMergedRegion(cras);
        }
        /// <summary>
        /// 填充表头数据
        ...
        {
            FileDataTitleToSheet(workbook, dataList, sheet);
            FileDataTimeToSheet(workbook, dataList, sheet);
            //填充表头
            if (dataList.Propertys.Count > 0)
            {
                ...
                foreach (var key in dataList.Propertys)
                {
                    ...
                    AutoColumnWidth(sheet,index);
                    //设置单元格样式
                    cell.CellStyle = NpoiHelper2.CreateHeadCellStyle(workbook, sheet, 11);
             ...
             where T : class
        {
            ...
            sheet.CreateFreezePane(3,0, 4, 3);
        }
